...
# 获取全部笔迹的完整窗口数据
def get_all_window_data(card_id):
    """从全部笔迹数据中获取完整的窗口大小数据
        ...
        if all_strokes:
            try:
                ...
                if 'window_size' in data:
                    ...
                    if isinstance(window_data, dict) and 'width' in window_data and 'height' in window_data:
                        print(f"Debug - 获取全部笔迹窗口数据: 找到窗口数据 {window_data}")
                        return window_data
            except Exception as e:
                print(f"解析全部笔迹获取窗口数据时出错: {e}")
                import traceback
                traceback.print_exc()
        
        # 没有找到窗口大小信息
        print(f"Debug - 获取全部笔迹窗口数据: 未找到窗口数据")
        return None
    except Exception as e:
        print(f"获取全部笔迹窗口数据时出错: {e}")
        import traceback
        traceback.print_exc()
        return None
