...
toolbar_buttons_config = {
    ...
    'flip_to_question': True  # 翻转到问题侧按钮
}
...
class ToolbarControlDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # 翻转到问题侧按钮
        self.checkboxes['flip_to_question'] = QCheckBox(lang.get_text("dialog_flip_to_question_button", "Flip to Question Button"))
        tools_layout.addWidget(self.checkboxes['flip_to_question'])
...
def apply_toolbar_config(editor=None, html=None):
    ...
    js_code += f"const btnFlipToQuestion = document.getElementById('ts_flip_to_question_button');\n"
    js_code += f"if (btnFlipToQuestion) btnFlipToQuestion.style.display = {str(toolbar_buttons_config['flip_to_question']).lower()} ? 'block' : 'none';\n\n"
