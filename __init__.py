...
def bridge_command(cmd):
    ...
    if cmd.startswith("ankidraw:save_eraser_size:"):
    ...
    elif cmd == "ankidraw:flip_to_question":
        # 处理翻转到问题侧的命令
        try:
            print("Debug - 收到翻转到问题侧命令")
            if mw.reviewer and hasattr(mw.reviewer, '_showQuestion'):
                mw.reviewer._showQuestion()
                print("Debug - 成功执行翻转到问题侧")
            else:
                print("Debug - 翻转到问题侧失败，reviewer或_showQuestion不可用")
        except Exception as e:
            print(f"执行翻转到问题侧时出错: {e}")
            import traceback
            traceback.print_exc()
        ...
        # 格式: ankidraw:restore_all_window_size:[cardId]:[dpr]:[osType]:[currentWidth]:[currentHeight]:[docHeight]:[cardHeight]
        try:
            # 分割命令，提取卡片ID和系统信息
            parts = cmd.split(":", 3)
            card_id = parts[2]
            ...
            dpr = 1.0
            is_windows = False
            current_width = current_height = doc_height = card_height = None
            ...
            if len(parts) > 3 and ':' in parts[3]:
                ...
                if len(sys_info) >= 2:
                    try:
                        ...
                        # 获取额外的窗口信息
                        if len(sys_info) >= 6:
                            current_width = int(sys_info[2])
                            current_height = int(sys_info[3])
                            doc_height = int(sys_info[4])
                            card_height = int(sys_info[5])
                    except Exception as e:
                        print(f"解析窗口恢复信息时出错: {e}")
            
            print(f"Debug - 恢复全部笔迹窗口大小: 请求卡片ID={card_id}, DPR={dpr}, 是否Windows={is_windows}")
            if current_width and current_height:
                print(f"Debug - 当前窗口信息: 宽度={current_width}, 高度={current_height}, 文档高度={doc_height}, 卡片高度={card_height}")
            
            # 获取保存的窗口大小信息
            window_data = stroke_storage.get_all_window_data(card_id)
            
            if window_data:
                print(f"Debug - 恢复全部笔迹窗口大小: 找到窗口数据 {window_data}")
                width = window_data.get('width')
                height = window_data.get('height')
                
                # 获取额外的窗口大小信息（如果有）
                saved_dpr = window_data.get('dpr', 1.0)
                base_height = window_data.get('baseHeight')
                saved_doc_height = window_data.get('docHeight')
                body_height = window_data.get('bodyHeight')
                saved_card_height = window_data.get('cardHeight')
                original_height = window_data.get('originalHeight')
                was_windows = window_data.get('isWindows', False)
                
                print(f"Debug - 恢复全部笔迹窗口大小: 详细窗口信息 "
                      f"原始宽高=({width}, {height}), 保存时DPR={saved_dpr}, 当前DPR={dpr}, "
                      f"基准高度={base_height}, 文档高度={saved_doc_height}, body高度={body_height}, "
                      f"卡片高度={saved_card_height}, 原始窗口高度={original_height}, "
                      f"保存时是否Windows={was_windows}, 当前是否Windows={is_windows}")
                
                if width and height:
                    # 智能调整：使用当前卡片内容高度来辅助决定窗口高度
                    if card_height and saved_card_height and card_height > 0 and saved_card_height > 0:
                        # 计算卡片内容高度比例
                        card_ratio = card_height / saved_card_height
                        # 如果比例有显著变化，则调整高度
                        if abs(card_ratio - 1.0) > 0.1:  # 超过10%的变化
                            adjusted_height = int(height * card_ratio)
                            print(f"Debug - 恢复全部笔迹窗口大小: 基于卡片内容比例调整高度，从{height}调整到{adjusted_height}")
                            height = adjusted_height
                    
                    # Windows系统下的特殊处理逻辑
                    if is_windows:
                        # 如果保存时已经进行了Windows补偿，而恢复时也是Windows，则不需要额外补偿
                        if was_windows:
                            # 只需要处理DPI变化带来的差异
                            if dpr != saved_dpr and dpr > 0 and saved_dpr > 0:
                                dpi_ratio = dpr / saved_dpr
                                # 只在DPI比率变化显著时调整
                                if abs(dpi_ratio - 1.0) > 0.05:
                                    adjusted_height = int(height * dpi_ratio)
                                    print(f"Debug - 恢复全部笔迹窗口大小: DPI调整，从{height}调整到{adjusted_height}")
                                    height = adjusted_height
                        else:
                            # 保存时不是Windows，但恢复时是Windows，需要补偿
                            # 根据当前DPI选择合适的补偿系数
                            compensation_factor = 1.05  # 基础补偿5%
                            if dpr > 1.5:  # 高DPI显示器
                                compensation_factor = 1.08  # 增加8%
                            elif dpr > 1.0:
                                compensation_factor = 1.06  # 增加6%
                            
                            adjusted_height = int(height * compensation_factor)
                            print(f"Debug - 恢复全部笔迹窗口大小: Windows系统补偿，从{height}调整到{adjusted_height}")
                            height = adjusted_height
                    elif was_windows:
                        # 保存时是Windows，但恢复时不是，可能需要减少高度
                        # 简单移除之前添加的补偿
                        compensation_factor = 0.95  # 减少5%
                        adjusted_height = int(height * compensation_factor)
                        print(f"Debug - 恢复全部笔迹窗口大小: 非Windows系统减少补偿，从{height}调整到{adjusted_height}")
                        height = adjusted_height
                    
                    # 确保最小高度
                    min_height = 400
                    if height < min_height:
                        print(f"Debug - 恢复全部笔迹窗口大小: 高度低于最小值，从{height}调整到{min_height}")
                        height = min_height
                    ...
                    print(f"Debug - 恢复全部笔迹窗口大小: 窗口数据缺少宽度或高度")
                    from aqt.utils import tooltip
                    tooltip(lang.get_text("restore_window_size_not_found"))
            else:
        ...
        try:
            ...
            # 新设计：不再加载问题侧笔迹
            print(f"Debug - 加载正面笔迹: 问题侧笔迹功能已禁用")
        except Exception as e:
        ...
        try:
            ...
            if is_question_side:
                # 新设计：问题侧不加载笔迹
                print(f"Debug - 兼容模式加载笔迹: 问题侧不加载笔迹")
            else:
                stroke_data = stroke_storage.load_all_stroke_data(card_id)
                side_name = "全部"
...
# 修改load_card_strokes函数，不再加载正面笔迹
def load_card_strokes():
    ...
    # 新设计：不再加载正面笔迹数据
    # 仅调整画布大小并通知JavaScript更新按钮状态
    resize_js()
    
    # 触发自定义事件，通知JavaScript更新按钮状态
    execute_js("document.dispatchEvent(new CustomEvent('ankidraw:showQuestion'));")
...
def load_answer_strokes():
    ...
    # 触发自定义事件，通知JavaScript更新按钮状态
    execute_js("document.dispatchEvent(new CustomEvent('ankidraw:showAnswer'));")
    
    # Bug修复：在答案侧笔迹加载完成后立即触发保存操作
    # 延迟执行保存操作，确保笔迹加载和合并完成
    from aqt.qt import QTimer
    QTimer.singleShot(1000, lambda: execute_js("if (typeof save_strokes === 'function') { save_strokes(); console.log('AnkiDraw Debug: 答案侧自动保存笔迹'); }"))
...
def ts_onload():
    ...
    addHook("showQuestion", load_card_strokes)  # 显示问题时设置问题侧状态
    addHook("showAnswer", load_answer_strokes)  # 显示答案时加载笔迹
...
def blackboard():
    ...
    flip_to_question_js = """
    // 定义全局变量用于跟踪是否在问题侧
    var is_question_side = true;
    
    // 翻转到问题侧函数
    function flip_to_question() {
        try {
            // 只有在答案侧时才启用此功能
            if (!is_question_side) {
                console.log('AnkiDraw Debug: 翻转到问题侧');
                
                // 通过pycmd发送命令到Python端
                if (typeof pycmd === 'function') {
                    pycmd('ankidraw:flip_to_question');
                } else {
                    console.error('AnkiDraw Error: pycmd函数不可用，无法翻转到问题侧');
                }
            }
        } catch (e) {
            console.error('AnkiDraw Error: 翻转到问题侧时出错', e);
        }
    }
    
    // 更新翻转按钮状态的函数
    function update_flip_button_state(is_on_question_side) {
        const flipButton = document.getElementById('ts_flip_to_question_button');
        if (flipButton) {
            is_question_side = is_on_question_side;
            if (is_question_side) {
                // 如果在问题侧，禁用按钮
    ...
    // 注册事件监听器，当showQuestion事件触发时更新按钮状态
    document.addEventListener('ankidraw:showQuestion', function() {
        update_flip_button_state(true);
    });
    
    // 注册事件监听器，当showAnswer事件触发时更新按钮状态
    document.addEventListener('ankidraw:showAnswer', function() {
        update_flip_button_state(false);
    });
    ...
    # 将翻转到问题侧函数添加到JS内容中
    js_content = js_content + flip_to_question_js
    ...
    # 添加翻转到问题侧按钮的多语言支持
    html_content = html_content.replace('title="Flip to question side"', 
                                       f'title="{lang.get_text("tooltip_flip_to_question", "Flip to question side")}"')
...
def ts_setup_menu():
    ...
    # 添加透明度设置到笔设置上方
    tool_submenu.addAction(ts_menu_opacity)
...
def restore_writing_window_size():
    ...
    try:
        ...
        if not card_id:
            ...
            showInfo(lang.get_text("restore_window_size_no_card", "没有正在复习的卡片，无法恢复窗口大小。"))
            return

        # 执行JavaScript函数恢复窗口大小
        execute_js("if (typeof restore_window_size === 'function') { restore_window_size(); }")
    except Exception as e:
        print(f"恢复窗口大小时出错: {e}")
        import traceback
        traceback.print_exc()
